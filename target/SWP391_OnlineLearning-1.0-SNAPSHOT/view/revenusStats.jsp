...
<html lang="en">
    ...
    <body class="ttr-opened-sidebar ttr-pinned-sidebar">
        ...
        <div class="right-sider">
            ...
            <div class="row" style="margin: 20px">
                    ...
                    <div class="widget-box">
                        ...
                        <div class="widget-inner">
                            <div class="search-btn" style="margin-bottom: 20px" >
                                <form method="get" action="revenusStats"">
                                    From: <input type="date" name="from" value="${requestScope.from}">
                                    To: <input type="date" name="to" value="${requestScope.to}">
                                    Total income: <fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${requestScope.totalRevenus}" />
                                    <input class="btn" type="submit" value="View" style="margin-left: 1rem">
                                </form>
                                <button id="export-btn1" class="btn">Export</button>
                            </div>
                            ...
                            <c:if test="${requestScope.registrationList.size()>0 and requestScope.registrationList ne null}">
                                <table border="1px" class="table-sortable">
                                    ...
                                    <tbody>
                                        <c:forEach items="${requestScope.registrationList}" var="r" varStatus="loop">
                            ...
                            <c:if test="${requestScope.registrationList eq null or requestScope.registrationList.size() <= 0}">
                                ...
                                <h3 class="not_found">No revenue found!</h3>
                            </c:if>
                        </div>
                    </div>
                ...
                <table border="1px" id="revenue_table" style="display: none">
                    <thead>
                        <tr>
                            <th style="text-align: center">No</th>
                            <th style="text-align: center">Email</th>
    ...
    <script>
        var btnXlsx = document.getElementById('export-btn1');

        btnXlsx.onclick = () => exportData('xlsx')

        function exportData(type) {
            const fileName = 'revenue-sheet.' + type
            const revenue_table = document.getElementById("revenue_table")
            const wb = XLSX.utils.table_to_book(revenue_table)
    ...
    <script>
        ...
        function renderPaging(id, pageIndex, totalPage, gap, from, to) {
            ...
            if (pageIndex - gap > 1) {
                container.innerHTML += '<a href="revenusStats?pageIndex=1' + '&from=' + from + '&to=' + to + '">First</a> ';
            }
            for (var i = pageIndex - gap; i < pageIndex; i++) {
                if (i > 0) {
                    container.innerHTML += '<a href="revenusStats?pageIndex=' + i + '&from=' + from + '&to=' + to + '">' + i + '</a>';
                }
            }
            ...
            if (pageIndex + gap < totalPage) {
                container.innerHTML += '<a href="revenusStats?pageIndex=' + totalPage + '&from=' + from + '&to=' + to + '">Last</a>';
            }
        }
