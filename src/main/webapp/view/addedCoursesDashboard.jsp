    ...
    <body>
                ...
                <form action="addedCourse?action=${requestScope.action eq null ? "add": requestScope.action}" method="post" enctype="multipart/form-data" id="addCourse" >
                    ...
                    <div class="top" style="margin-top: 2%; display: flex;">
                        <div class="top-left" style="width: 30%;">
                            <img style="margin-left: 20%;width: 480px; height: 480px; object-fit: contain" src="${requestScope.courseEdit.img eq null ? "../assets/images/anh_1.jpg":requestScope.courseEdit.img}" alt="" id="imagePreview" onclick="triggerFileInput()"/>
                            <input type="file" name="imageFile" accept="image/*" style="display: none" id="uploadInput" onchange="uploadImage(event)">
                        </div>
                        <div class="top-right" style="width: 60%; margin-left: 10%; margin-top: 5%;">
                            <table style="margin-bottom: 0">
                                <tbody>
                                    <tr>
                                        <td style="width: 130px;">Course name: </td>
                                        <td><input style="width: 280px; padding: 5px;" type="text" id="courseName" name="courseName" onclick="hideError()" placeholder="Course name" value="${requestScope.name eq null ? requestScope.courseEdit.name : requestScope.name}"></td>
                                    </tr>
                                    <tr>
                                        <td>Price: </td>
                                        <td><input style="width: 280px; padding: 5px;" type="text" id="price" name="cost" onclick="hideError()" placeholder="Price of course" value="${requestScope.price eq null ? requestScope.courseEdit.price : requestScope.price}"></td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <table style="width: 88%;">
                                <tbody>
                                        ...
                                        <td style="width: 280px;">
                                            <select name="category">
                                                <c:forEach items="${requestScope.cate}" var="c">
                                                    <option value="${c.courseCategoryId}" ${requestScope.category ne null? requestScope.category eq c.courseCategoryId ? "selected":"" :requestScope.courseEdit.category.courseCategoryId eq c.courseCategoryId?"selected":""}>${c.name}</option>
                                                </c:forEach>
                                            </select>
                                        </td>
                                        <td style="width: 115px;">Level: </td>
                                        <td>
                                            <select name="level">
                                                <c:forEach items="${requestScope.levelList}" var="l">
                                                    <option value="${l.levelId}" ${requestScope.level ne null ?requestScope.level eq l.levelId ? "selected":"":requestScope.courseEdit.level.levelId eq l.levelId ? "selected":""}>${l.name}</option>
                                                </c:forEach>
                                            </select>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Duration: </td>
                                        <td>
                                            <select style="width: 280px;" name="duration">
                                                <c:forEach items="${requestScope.durationList}" var="d">
                                                    <option value="${d.durationId}" ${requestScope.duration ne null ?requestScope.duration eq d.durationId ? "selected":"":requestScope.courseEdit.duration.durationId eq d.durationId?"selected":""}>${d.name}</option>
                                                </c:forEach>
                                            </select>
                                        </td>
                                        <td>Language: </td>
                                        <td>
                                            <select name="language">
                                                <c:forEach items="${requestScope.languageList}" var="la">
                                                    <option value="${la.languageId}" ${requestScope.language ne null ?requestScope.language eq la.languageId ? "selected":"": requestScope.courseEdit.language.languageId eq la.languageId? "selected":""}>${la.name}</option>
                                                </c:forEach>
                                            </select>
                                        </td>
                                    </tr>
                    ...
                    <div style="font-size: 18px; margin: 20px 0 10px 6%;">Description:</div>
                    ...
                    <div id="error" style="color:red;margin-left: 5.5%; margin-bottom: 10px;"></div>
                    <c:if test="${requestScope.mesString ne null}">
                        <div id="mesString" style="color:green;margin-left: 5.5%; margin-bottom: 10px;">${requestScope.mesString}</div>
                    </c:if>
                    <c:if test="${requestScope.courseEdit.img eq null}"> 
        ...
        <script>
            ...
            function sendForm() {
                ...
                if (name.length === 0) {
                    error = 'Course name is invalid. Please try again';
                    document.getElementById('error').textContent = error;
                    document.getElementById('error').style.display = 'block';
                } else if (!price.match('^\\d+$')) {
                    error = 'Price of course is invalid. Please try again';
                    document.getElementById('error').textContent = error;
                    document.getElementById('error').style.display = 'block';
                } else {
